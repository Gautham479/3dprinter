    ...
    return (
      <div className="min-h-screen bg-surface-bg flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <motion.div
            className="w-16 h-16 rounded-sm border-2 border-primary-500/30 border-t-primary-500"
            animate={{ rotate: 360 }}
    ...
    return (
      <div className="min-h-screen bg-surface-bg flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <motion.div
          ...
          className="relative rounded-sm border border-red-500/20 bg-surface-card/80 backdrop-blur-xl p-10 max-w-md text-center shadow-2xl"
        >
    ...
    <div className="min-h-screen py-10 font-sans text-fg relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 relative">
        ...
        <motion.div
          ...
          className={`relative rounded-sm border overflow-hidden ${isPaid ? 'border-green-500/30' : 'border-red-500/30'
        ...
        >
          {/* Top accent */}
          <div className={`absolute top-0 left-0 right-0 h-[3px] ${isPaid ? 'bg-green-500' : 'bg-red-500'}`} />
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 justify-between">
            <div className="flex flex-col md:flex-row items-center gap-6">
              <motion.div
                ...
                className={`w-20 h-20 rounded-sm flex items-center justify-center flex-shrink-0 ${isPaid ? 'bg-green-500/15 border border-green-500/30' : 'bg-red-500/15 border border-red-500/30'
                  }`}
              ...
              <div>
                ...
                {isPaid ? (
                  <>
                    ...
                    <div className="mt-4 bg-primary-500/10 border border-primary-500/20 text-primary-500 px-4 py-3 rounded-sm text-sm">
                ...
                <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-sm bg-surface-muted/60 border border-surface-border text-sm font-bold text-fg-muted">
                  Order <span className="text-primary-500 uppercase">{order.orderId}</span>
                </div>
              </div>
            </div>

            <button className="flex items-center gap-2 px-5 py-3 bg-surface-muted/60 hover:bg-surface-muted border border-surface-border text-fg font-bold rounded-sm transition-colors flex-shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <motion.div
              ...
              className="rounded-sm border border-surface-border/60 bg-surface-card/70 backdrop-blur-xl p-6 relative overflow-hidden"
            >
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`font-black px-2.5 py-1 rounded-sm text-xs ${isPaid ? 'bg-green-500/15 text-green-400 border border-green-500/20' : 'bg-red-500/15 text-red-400 border border-red-500/20'
                    }`}>
                    {order.status}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center py-2 border-b border-surface-border/40">
                  ...
                  <span className="font-bold text-fg text-xs bg-surface-muted px-2 py-1 rounded-sm">{order.razorpayPaymentId || "N/A"}</span>
                </div>
                <div className="flex justify-between items-center bg-primary-500/10 border border-primary-500/20 p-3 rounded-sm mt-2">
                  <span className="text-primary-500 font-black">Amount Paid:</span>
                  <span className="text-xl font-black text-primary-500">₹{order.totalAmount}</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <h2 className="text-lg font-black mb-5 flex items-center gap-2 border-b border-surface-border/50 pb-4">
                <div className="w-8 h-8 rounded-sm bg-accent-500/15 border border-accent-500/30 flex items-center justify-center">
          ...
          <motion.div
            ...
            className="rounded-sm border border-surface-border/60 bg-surface-card/70 backdrop-blur-xl p-6 h-fit relative overflow-hidden"
          >
            ...
            <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1 custom-scrollbar">
              {items && items.map((item, idx) => (
                <motion.div
                  ...
                  className="flex justify-between items-start bg-surface-muted/40 border border-surface-border/60 p-4 rounded-sm"
                >
            ...
            <div className="mt-6 pt-5 border-t border-surface-border/50 space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center text-fg-muted">
                <span>Delivery</span>
                {order.deliveryFee === 0 ? (
                  <span className="text-green-400 font-black text-xs bg-green-400/10 border border-green-400/20 px-2 py-0.5 rounded-sm">FREE</span>
                ) : (
              ...
              <div className="pt-4 border-t border-surface-border/50 border-dashed flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-primary-500">₹{order.totalAmount}</span>
              </div>
            </div>
          </motion.div>
        </div>
