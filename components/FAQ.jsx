...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="w-full py-20 bg-surface-bg border-b border-surface-border">
        ...
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <motion.div
              ...
              className={`border transition-colors duration-200 ${openIndex === i
                  ? 'border-surface-border bg-surface-card'
                  : 'border-surface-border bg-surface-bg hover:bg-surface-muted'
                }`}
            >
              <button
              ...
              >
                <span className={`font-bold text-base pr-4 ${openIndex === i ? 'text-primary-600' : 'text-fg'}`}>
                  {faq.q}
                </span>
                <div
                  className={`flex-shrink-0 w-6 h-6 flex items-center justify-center transition-transform duration-200 ${openIndex === i ? 'rotate-180 text-primary-600' : 'text-fg-subtle'}`}
                >
